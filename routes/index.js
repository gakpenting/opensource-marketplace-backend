...
const {forSellRepo,allRepo,ownedRepo,profile}=require("./utils/for_sale");
const {sell_repo,unlist_repo,for_sale_repo}=require("./utils/sell_repo");
const {authorize_paypal,save_paypal,get_paypal,disconnect_paypal}=require("./utils/paypal");
...
router.get('/get-for-sale-repo',async function(req, res, next) {
   ...
   router.get('/for_sale_repo',async function(req, res, next) {
    const params={}
    params.token=req.query.token
    params.token_pass=process.env.token_pass;
    params.frontend_url=process.env.frontend_url;
   ...
   router.get('/manage-access/:token/:id/:random',async function(req, res, next) {
    const params={}
    params.token_pass=process.env.token_pass;
